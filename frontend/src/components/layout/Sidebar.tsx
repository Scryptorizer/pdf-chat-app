...
const navigation: NavigationItem[] = [
  { id: 'dashboard', name: 'Dashboard', href: '/', icon: '📊', badge: 'Live', badgeColor: 'bg-emerald-500', description: 'Business overview' },
  { id: 'events', name: 'Active Events', href: '/events', icon: '🎯', badge: '8', badgeColor: 'bg-blue-500', description: 'Event management' },
  { id: 'hotels', name: 'Hotel Partners', href: '/hotels', icon: '🏨', description: 'Partner network' },
  { id: 'analytics', name: 'Analytics', href: '/analytics', icon: '📈', description: 'Business intelligence' },
  { id: 'chat', name: 'AI Assistant', href: '/chat', icon: '🤖', badge: 'AI', badgeColor: 'bg-purple-500', description: 'Intelligent insights' },
  { id: 'settings', name: 'Settings', href: '/settings', icon: '⚙️', description: 'Platform config' }
];

export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onToggle }) => {
  const location = useLocation();
  const navigate = useNavigate();
  const drawerRef = useRef<HTMLDivElement>(null);        // 👈  new ref

  /* ==========  ADD-ON: ESC to close + focus trap (mobile only)  ========== */
  useEffect(() => {
    if (!isOpen) return;
  ...
  const handleNavigation = (href: string) => {
    navigate(href);
    if (window.innerWidth < 1024) onToggle(); // auto-close on phones
  };

  return (
    <div
      ref={drawerRef} tabIndex={-1} aria-modal="true"                /* 👈  ref & a11y */
      className={`
        fixed inset-y-0 left-0 z-50
        w-72 max-w-[80%] lg:w-64          /* 👈  wider phones, unchanged on lg */
          ...
          return (
            <div key={item.id} className="relative group">
              <button
                ...
                `}
              >
                {isActive && <div className="absolute left-0 top-0 bottom-0 w-1 bg-white rounded-r-full shadow-lg"></div>}
                <span className={`text-xl mr-4 transition-transform ${isActive ? 'scale-110' : 'group-hover:scale-105'}`}>
                  {item.icon}
                </span>
                ...
                {item.badge && (
                  <span
                    className={`ml-3 px-2.5 py-1 text-xs font-bold rounded-full text-white ${item.badgeColor || 'bg-slate-600'}
                      ${isActive ? 'bg-white/20 shadow-lg' : ''} animate-pulse`}
                  >
                    {item.badge}
                  </span>
                )}
      ...
      <div className="p-4 border-t border-slate-700/50 bg-slate-800/30">
        <div className="space-y-3">
          <div className="flex justify-between items-center text-xs">
            ...
            <span className="text-emerald-400 font-bold bg-emerald-500/10 px-2 py-1 rounded-full">$835K</span>
          </div>
          <div className="flex justify-between items-center text-xs">
            <span className="text-slate-400 font-medium">Active Events</span>
            <span className="text-blue-400 font-bold bg-blue-500/10 px-2 py-1 rounded-full">8</span>
          </div>
          <div className="flex justify-between items-center text-xs">
            <span className="text-slate-400 font-medium">Win Rate</span>
            <span className="text-purple-400 font-bold bg-purple-500/10 px-2 py-1 rounded-full">41.8%</span>
          </div>
        </div>
      </div>
